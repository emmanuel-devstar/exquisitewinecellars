...
function remove_posts_menu_item()
{
  ...
  $panel = $options['panel'] ?? [];
  if (empty($panel['posts'])) remove_menu_page('edit.php'); // 'edit.php' corresponds to the Posts menu
}
...
function remove_custom_post_menu()
{
  $options = get_fields("option");
  $panel = $options['panel'] ?? [];
  if (empty($panel['news'])) remove_menu_page('edit.php?post_type=news');
  if (empty($panel['services'])) remove_menu_page('edit.php?post_type=services');
  if (empty($panel['case_studies'])) remove_menu_page('edit.php?post_type=case-studies');
}
